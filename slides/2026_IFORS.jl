...
# ╔═╡ 5bb364d5-4068-4e10-8a6b-7a553eda75dc
include(joinpath(dirname(dirname(pathof(ArrayDiff))), "perf", "acopf", "structured.jl"))
...
# ╔═╡ 469ae78e-0ca2-4bd0-82f5-6dceba134fde
md"## Angle difference limit"
...
# ╔═╡ 6eeddac3-6069-4eb5-9bc3-5cc6347f3611
md"## Branch flows"

# ╔═╡ cdaa0fc0-fe84-440d-84bd-d4f28c1dd8e5
md"## Nodal power balance"

# ╔═╡ 40085733-43fd-4d7e-a885-19b4ae3e4d9b
md"## Thermal limit"
...
* Speed up MOI wrapper of ExaModels
  - [MathOptSymbolicAD](https://github.com/lanl-ansi/MathOptSymbolicAD.jl) can cheaply detect similar expressions
* Second-order / Hessian for ArrayDiff
* Maturation phase for both GenOpt and ArrayDiff: feedback welcome!
"""
...
import PGLib, PowerModels, ExaModels, MadNLP, MadNLPGPU
...
if CUDA.functional()
	set_optimizer(model, () -> ExaModels.Optimizer(MadNLP.madnlp, CUDABackend()))
	optimize!(model)
end

# ╔═╡ c8038d45-05cf-46c9-b194-8e7647ea3593
function parse_polar_case(ref)
...
d = parse_polar_case(ref)

# ╔═╡ 483ebe34-e0ba-44d9-927f-fe370d4f678a
begin
    nb, ng, nl = d.nbus, d.ngen, d.nbranch
    array_model = JuMP.Model()
    JuMP.@variable(array_model, Va[1:nb], container = ArrayDiff.ArrayOfVariables)
    JuMP.@variable(array_model, Vm[1:nb], container = ArrayDiff.ArrayOfVariables)
    JuMP.@variable(array_model, Pg[1:ng], container = ArrayDiff.ArrayOfVariables)
    JuMP.@variable(array_model, Qg[1:ng], container = ArrayDiff.ArrayOfVariables)
    JuMP.@variable(array_model, pf[1:nl], container = ArrayDiff.ArrayOfVariables)
    JuMP.@variable(array_model, pt[1:nl], container = ArrayDiff.ArrayOfVariables)
    JuMP.@variable(array_model, qf[1:nl], container = ArrayDiff.ArrayOfVariables)
    JuMP.@variable(array_model, qt[1:nl], container = ArrayDiff.ArrayOfVariables)
end

# ╔═╡ a9ed467a-ffe9-4fdd-a232-f1d8b1bdf6be
Fg = GatherMatrix(d.f_bus, nb)

# ╔═╡ af30f5e4-8e33-4f55-ac6d-2c714c2b8ef0
Tg = GatherMatrix(d.t_bus, nb)

# ╔═╡ dabb2db9-e088-41ac-b276-5688443ac557
Cg = GatherMatrix(d.gen_bus, nb)

# ╔═╡ ebb02343-fb71-4861-bcc2-d36ebb0ab79f
Ft, Tt, Ct = transpose(Fg), transpose(Tg), transpose(Cg);

# ╔═╡ 7362d83a-8242-4b1a-8fe0-b2d2ce9dbe54
begin
vmf, vmt = Fg * Vm, Tg * Vm
Δ = Fg * Va .- Tg * Va
cΔ, sΔ = cos.(Δ), sin.(Δ)
...
# ╔═╡ c80376b1-8be1-4957-b681-6df0e01f94c6
begin
    JuMP.@constraint(array_model, pf .- (d.c5 .* vmf .^ 2 .+ d.c3 .* (vv .* cΔ) .+ d.c4 .* (vv .* sΔ)) in MOI.Zeros(nl))
    JuMP.@constraint(array_model, qf .+ d.c6 .* vmf .^ 2 .+ d.c4 .* (vv .* cΔ) .- d.c3 .* (vv .* sΔ) in MOI.Zeros(nl))
    JuMP.@constraint(array_model, pt .- (d.c7 .* vmt .^ 2 .+ d.c1 .* (vv .* cΔ) .- d.c2 .* (vv .* sΔ)) in MOI.Zeros(nl))
    JuMP.@constraint(array_model, qt .+ d.c8 .* vmt .^ 2 .+ d.c2 .* (vv .* cΔ) .+ d.c1 .* (vv .* sΔ) in MOI.Zeros(nl))
end;

# ╔═╡ c76dfbcd-3a9e-4dd3-a436-75b8abcb3e2d
begin
    JuMP.@constraint(array_model, Ft * pf .+ Tt * pt .- Ct * Pg .+ d.pd .+ d.gs .* Vm .^ 2 in MOI.Zeros(nb))
    JuMP.@constraint(array_model, Ft * pf .+ Tt * pt .- Ct * Qg .+ d.qd .- d.bs .* Vm .^ 2 in MOI.Zeros(nb))
end;

# ╔═╡ aeb3615c-a5b1-42cc-b77e-0d7ee8193dfc
begin
    JuMP.@constraint(array_model, pf .^ 2 .+ qf .^ 2 .- d.rate_a_sq in MOI.Nonpositives(nl))
    JuMP.@constraint(array_model, pt .^ 2 .+ qt .^ 2 .- d.rate_a_sq in MOI.Nonpositives(nl))
end;

# ╔═╡ 7b2bb794-05b6-4e97-bdf1-6869b3c070ee
JuMP.@objective(array_model, Min, sum(d.cost1 .* Pg .^ 2 .+ d.cost2 .* Pg));
